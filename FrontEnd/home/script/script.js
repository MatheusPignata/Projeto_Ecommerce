...
function limparTela(){
    let aaa = document.querySelectorAll(".prodCard");
...
function buscar() {
    let val = document.querySelector(".input-search").value.toLowerCase();
    let prods = document.querySelectorAll(".prodCard")

    for(let i = 0; i < prods.length; i++){
        if (prods[i].innerHTML.toString().toLowerCase().includes(val)) {
            prods[i].classList.remove("model");
        }else{
            prods[i].classList.add("model");
        }
    }
...
function cadastro() {
    if(user.innerHTML == "") window.location.href = "../login_cadastro/index.html";
            ...
            if(pedido.cliente == obj.id_cliente){
                ...
                pedido.itens.forEach(item => {
                    ...
                    if(item.produto.promocao == 0){
                        itemPedido.querySelector(".valor").innerHTML = "R$ " + (item.produto.valor).toFixed(2);
                    }else{
                        itemPedido.querySelector(".valor").innerHTML = "R$ " + (item.produto.promocao).toFixed(2);
                    }
